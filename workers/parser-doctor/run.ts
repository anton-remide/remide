...
async function findBrokenParsers(lookbackHours: number, specificParser: string | null): Promise<BrokenParser[]> {
  ...
  for (const row of data ?? []) {
    ...
    if (!parserFile) {
      logger.info(SCOPE, `Skipping ${rid} — no parser file in registries/`);
      continue;
    }

    const sourceUrl = extractSourceUrl(parserFile);
...
function applyFixAndCommit(registryId: string, fixedCode: string, parserFile: string, diagnosis: string): string | null {
  ...
  try {
    ...
    fs.writeFileSync(parserFile, fixedCode, 'utf-8');
    const relPath = path.relative(repoRoot, parserFile);
    execSync(`git add "${relPath}"`, { cwd: repoRoot, stdio: 'pipe' });
    const diagSafe = diagnosis.slice(0, 200).replace(/"/g, "'").replace(/\n/g, ' ');
    execSync(
      `git commit -m "fix(parser): auto-fix ${registryId} — parser-doctor" -m "Diagnosis: ${diagSafe}"`,
      { cwd: repoRoot, stdio: 'pipe' }
    );
    execSync(`git push origin ${branch}`, { cwd: repoRoot, stdio: 'pipe' });

    const prBody = [
      '## Parser Doctor Auto-Fix',
      '',
      `**Parser:** ${registryId}`,
    ...
    const prBodyFile = path.join(repoRoot, '.pr-body-tmp.md');
    fs.writeFileSync(prBodyFile, prBody, 'utf-8');
    const prUrl = execSync(
      `gh pr create --title "fix(parser): auto-fix ${registryId}" --body-file .pr-body-tmp.md --base main`,
