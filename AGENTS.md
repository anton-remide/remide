...
|--------|------|-------|----------|
| **Anton** | Owner, Full-Stack Lead | Backend: `parsers/`, `workers/`, `shared/`, `scripts/`, `.github/`, `AGENTS.md`. Frontend: `src/data/`, `src/types.ts`, `src/pages/` (structure + data), `src/App.tsx` (routes) | `backend/*` |
...
- Conflicts in shared files (`AGENTS.md`, `package.json`) — resolve together
...
- `/vasp-tracker-Codex-analyze-crypto-registries-CD5TT/` — Old abandoned codebase. Only data files carried over. Do not modify.
...
| `notion-sync.mdc` | Always | Session-end checklist: Workers Registry, KB, Current State, AGENTS.md |
| `version-guard.mdc` | package.json, vite.config, App.tsx, app.css | When infra files change → verify AGENTS.md Tech Stack accuracy |
...
**5 фаз:**
0. **Task Scan** — загрузить контекст (AGENTS.md, код, Notion)
...
**MD files = fast cache.** Quick-start context for Codex, synced after every session.
...
| Project rules & constraints | **This file** (AGENTS.md) | Loaded automatically by Codex |
...
1. Read this file (AGENTS.md) — project rules, constraints, IDs
2. Read `MEMORY.md` from your Codex memory directory — cached status, quick context
3. Read `project-decisions.md` from memory — cached decisions
4. Check for plan files in your Codex plans directory
...
Properties:
  ...
  Owner           — SELECT: Codex, Anton, Sasha, Both
...
Each developer's Codex maintains its own local memory files. They are NOT shared via git — each agent builds its own context from Notion (source of truth).
